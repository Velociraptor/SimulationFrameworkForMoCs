...
int main() {
	...
	Platform *sensePlatform = new Platform(string("sensePlatform"), actuateTasks, actuateClk);
	...
	platforms.push_back(sensePlatform);
	platforms.push_back(actuatePlatform);
	vector<Network*> networks;
	networks.push_back(network);
	chrono::milliseconds timestart(0);
	PtidesDirector* ptidesD = new PtidesDirector(platforms, networks, timestart);
	...
	InterruptInvocation *rainfallIntInvoke = new InterruptInvocation(rainfallInterrupt, rainIntGuard, rainIntFreq, rainIntPriority);
