...
int main() {
	...
	Platform(string name, vector<PTask*> tasks, unsigned int clktime)
	...
	// Task invocation for rain generation based on weather model
	PTask *generateRainfall = new PTask(string("MakeItRain"), weatherModel);
	// Empty port vector for empty guards
	vector<Port*> trivialPV;
	Guard *rainGuard = new Guard(string("RainGuard"), trivialTrueGuard, trivialPV);
	unsigned int rainGenFreq = 200;
	unsigned int rainGenPriority = 1;
	PTaskInvocation *genRainInvoke = new PTaskInvocation(generateRainfall, rainGuard, rainGenFreq, rainGenPriority);
	...
	// Initialize rain sensor actor
	AccumulatorWithReset *rainfallSensor = new AccumulatorWithReset(string("RainfallSensor"), vipRainSense, vopRainSense);
	PTask *senseRainfall = new PTask(string("SenseTheRain"), rainfallSensor);
	Guard *sensorGuard = new Guard(string("SensorGuard"), trivialTrueGuard, trivialPV);
	unsigned int rainSenseFreq = 200;
	unsigned int rainSensePriority = 2;
	PTaskInvocation *senseRainInvoke = new PTaskInvocation(senseRainfall, sensorGuard, rainSenseFreq, rainSensePriority);
	...
	// Initialize rain register actor
	Register *prevRainRegister = new Register(string("PreviousRainRegister"), vipRainRegister, vopRainRegister);
	PTask *storeLatestRainfall = new PTask(string("StoreLatestRain"), prevRainRegister);
	Guard *rainRegGuard = new Guard(string("rainStoreGuard"), trivialTrueGuard, trivialPV);
	unsigned int rainStoreFreq = 200;
	unsigned int rainStorePriority = 3;
	PTaskInvocation *storeRainInvoke = new PTaskInvocation(storeLatestRainfall, rainRegGuard, rainStoreFreq, rainStorePriority);
	...
	// Initialize rain rate actor
	Difference *rainfallRateCheck = new Difference(string("RainfallRateCheck"),vipRainRate, vopRainRate);
	PTask *checkRainfallRate = new PTask(string("CheckRainRate"), rainfallRateCheck);
	Guard *rainRateGuard = new Guard(string("rainRateGuard"), trivialTrueGuard, trivialPV);
	unsigned int rainRateCheckFreq = 200;
	unsigned int rainRatePriority = 4;
	PTaskInvocation *rainRateInvoke = new PTaskInvocation(checkRainfallRate, rainRateGuard, rainRateCheckFreq, rainRatePriority);

	// Mode task invocations for generating and tracking rainfall
	vector<PTaskInvocation*> lowRainPTaskList;
	vector<PTaskInvocation*> medRainPTaskList;
	vector<PTaskInvocation*> highRainPTaskList;
	lowRainPTaskList.push_back(genRainInvoke);
	lowRainPTaskList.push_back(senseRainInvoke);
	lowRainPTaskList.push_back(storeRainInvoke);
	lowRainPTaskList.push_back(rainRateInvoke);
	medRainPTaskList.push_back(genRainInvoke);
	medRainPTaskList.push_back(senseRainInvoke);
	medRainPTaskList.push_back(storeRainInvoke);
	medRainPTaskList.push_back(rainRateInvoke);
	highRainPTaskList.push_back(genRainInvoke);
	highRainPTaskList.push_back(senseRainInvoke);
	highRainPTaskList.push_back(storeRainInvoke);
	highRainPTaskList.push_back(rainRateInvoke);
	...
	unsigned int lowWiperFreq = 25;
	unsigned int lowWiperPriority = 10;
	PTask *actuateWiperLow = new PTask(string("WipeItOffLow"), wiper);
	PTaskInvocation *lowWiperInvoke = new PTaskInvocation(actuateWiperLow, wiperGuard, lowWiperFreq, lowWiperPriority);
	lowRainPTaskList.push_back(lowWiperInvoke);
	unsigned int medWiperFreq = 50;
	unsigned int medWiperPriority = 10;
	PTask *actuateWiperMed = new PTask(string("WipeItOffMed"), wiper);
	PTaskInvocation *medWiperInvoke = new PTaskInvocation(actuateWiperMed, wiperGuard, medWiperFreq, medWiperPriority);
	medRainPTaskList.push_back(medWiperInvoke);
	unsigned int highWiperFreq = 100;
	unsigned int highWiperPriority = 10;
	PTask *actuateWiperHigh = new PTask(string("WipeItOffHigh"), wiper);
	PTaskInvocation *highWiperInvoke = new PTaskInvocation(actuateWiperHigh, wiperGuard, highWiperFreq, highWiperPriority);
	highRainPTaskList.push_back(highWiperInvoke);

	// Mode setup
	Mode *lowRainMode = new Mode(string("lowRainMode"), lowRainPTaskList);
	Mode *medRainMode = new Mode(string("medRainMode"), medRainPTaskList);
	Mode *highRainMode = new Mode(string("highRainMode"), highRainPTaskList);
	...
	// Initialize director and kick off simulation
